...
print("Number of category assignments:", sum(cats.values()), sum(cats.values())/N, sorted(tmp)[len(tmp)//2], max(tmp), sep="\t")
...
print("Number of paragraphs:", sum(paragraphs), sum(paragraphs) / N, paragraphs[len(paragraphs)//2], max(paragraphs), sep="\t")
print("Number of sentences:", sum(sentences), sum(sentences) / N, sentences[len(sentences)//2], max(sentences), sep="\t")
print("Number of words:", sum(tokens), sum(tokens) / N, tokens[len(tokens)//2], max(tokens), sep="\t")
print("Number of characters:", sum(chars), sum(chars) / N, chars[len(chars)//2], max(chars), sep="\t")
